...
2. `yarn format:check` — vérifie le formatage Prettier
3. `yarn type-check` — `vue-tsc`
4. `yarn build-only` — build de production
5. build de l'image Docker (sans push)

Reproduire la CI en local : `yarn format:check && yarn type-check && yarn build`.
